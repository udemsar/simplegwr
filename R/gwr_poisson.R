...
#'
#' This function calculates a Poisson gwr model, where dependent variable is a count variable
...
gwr_poisson <- function(formula, data, bw, kernel, weighting) {
  ...
  #-------  
  ## Step 2: calculate Poisson local models, one in each point 
  #------- 
  
  ## Select columns with only regression variables from data
  ...
  newNames <- c(newNames, 'LocRes', 'StLocRes', 'deviance', 'AIC','yP')
  #newNames
  ...
  # For each point get data, weigh them, calculate Poisson lm, extract values, add to data frame
  for (i in 1:noPoints) {
    ...
    #weightedData
    # Transfer dep variable back into integers, or Poisson won't work
    depVariable <- which(names(weightedData)==variables[1])
    weightedData[,depVariable] <- round(weightedData[,depVariable])
    # weightedData
    # Run Poisson regression on these weighted data to get the local model
    localM <- glm(formula, family = poisson(link = log), data = weightedData)
    #summary(localM)
    ...
    # 2. write statistics
    data$deviance[i] <- summary(localM)$deviance  
    data$AIC[i] <- summary(localM)$aic
    #head(data)
  ...
  ## calculate Deviance
  # Formula from here https://peijin.medium.com/the-poisson-deviance-for-regression-d469b56959ce
  # D = 2 * sum (y_i*log(y_i/yP_i)-(y_i-yP_i))
  # Find dependent variable
  yy <- data[,which(names(data)== variables[1])]
  # Find predicted variable
  yyP <- data$yP
  # Calculate deviance, calculation per row in these two vectors
